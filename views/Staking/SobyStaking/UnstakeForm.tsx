...
export default function UnStakeForm({ isOpen, onClose, stakingSMC }: Props) {
    ...
    const { userInfo, penaltyFee, onWithdraw, withdrawable, isSubmitWithdraw, pendingReward } = stakingSMC;

    const userAmountStaked = useMemo(() => {
        if (userInfo) {
            return Number(formatUnits(userInfo.amount, DECIMAL));
        }
        return 0;
    }, [userInfo]);
    ...
    const amountReceive = useMemo(() => {
        return unstakeAmount + pendingReward - penaltyFeeAmount;
    ...
    const onUnStaking = useCallback(async () => {
        await onWithdraw(parseUnits(unstakeAmount.toString(), DECIMAL));
        onClose();
    }, [unstakeAmount]);
        ...
        <div className="fixed inset-0 bg-[#000] bg-opacity-30 flex justify-center items-center top-0 left-0 z-50">
            <div className="w-[90%] max-w-[650px] bg-[#FFFFE5] py-10 px-7 rounded-3xl shadow-xl transition-transform transform-gpu scale-110 animate-zoomOut flex flex-col gap-3 border-b-[2px] border-brown">
                ...
                <div className="flex gap-4 items-center content-center">
                    ...
                    <div className="text-2xl font-bold">Unstake $SOBY</div>
                </div>
                <div className="flex flex-col gap-4">
                    <div className="flex justify-between w-full items-center content-center">
                        ...
                        <div className="flex gap-[10px] w-max">
                            <button
                                ...
                                onClick={() => setUnstakeAmount((userAmountStaked * 0.25).toString())}
                            >
                                25%
                            </button>
                            <button
                                className="h-10 leading-10 px-5 rounded-lg bg-[#FFEFC3]"
                                onClick={() => setUnstakeAmount((userAmountStaked * 0.5).toString())}
                            >
                                50%
                            </button>
                            ...
                            <button
                                className="h-10 leading-10 px-5 rounded-lg bg-[#FFEFC3]"
                                onClick={() => setUnstakeAmount((userAmountStaked * 0.75).toString())}
                            >
                                75%
                            </button>
                            <button
                                className="h-10 leading-10 px-5 rounded-lg bg-[#FFEFC3]"
                                onClick={() => setUnstakeAmount((userAmountStaked.toString()))}
                            >
                                Max
                            </button>
                        </div>
                    ...
                    <div className={cn("w-full p-4 bg-[#FFE49A] rounded-xl border-b-[2px] border-[#FFE49A] relative", {
                        ...
                        />
                        {!!error && <div className="text-[#e82b2b] text-[10px] font-bold absolute bottom-0">
                            * {error}
                        </div>
                        }
                    </div>
                    <div className="flex justify-between">
                        <label className="text-sm">My Staked Amount:</label>
                        <span className="text-lg">{FormatNumber(userAmountStaked)} $SOBY</span>
                    </div>
                    <div className="h-[2px] bg-[#FFE49A] w-full" />
                    <div className="bg-[#BF8D00] rounded-3xl py-6 px-7 flex flex-col gap-4">
                        {
                            !withdrawable && <div className="flex justify-between">
                                    ...
                                    <Tooltip
                                        ...
                                        title={
                                            <div className="flex p-2 flex-col gap-3 text-brown rounded-lg border-[1px] border-brown bg-[#FFEFC3] w-auto md:w-[320px]">
                                                <label className="text-sm font-bold">Early Unstake Fee: 2%</label>
                                                <div className="text-xs font-normal">The fee you will be charged if you unstake before the specified unlock time. The fee is <strong>2%</strong> of the unstake amount.</div>
                                            </div>
                                        }
                                    >
                                ...
                                <span className="text-lg font-bold text-[#CB0000]">
                                    {penaltyFeeAmount > 0 ? `${FormatNumber(penaltyFeeAmount)} $XAI - $SOBY` : `${penaltyFee}%`}
                                </span>
                            </div>
                        }
                        <div className="flex justify-between">
                            <label className="text-sm text-[#FFFFE5]">Reward:</label>
                            <span className="text-lg text-[#FFFFE5]">{FormatNumber(pendingReward)} $SOBY</span>
                        </div>
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-lg text-[#FFFFE5]">{FormatNumber(amountReceive)} $SOBY</span>
                        </div>
                    </div>
                </div>
